    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="bg-[#0a0a0c] silver-border max-w-xl max-h-[88vh] overflow-y-auto" data-testid="item-dialog">
        ...
        <div className="grid gap-3">
          ...
          {draft.isDaily ? (
            <Labeled label="Daily Use">
              <div className="flex items-center gap-2">
                <span className="font-meta text-xs text-[#E2E4E9] uppercase select-none">Daily Active</span>
                <button
                  onClick={() => apply({ ...draft, isDaily: false, isDailyUsed: false })}
                  className="p-2 silver-border bg-[#0d0d0f] hover:bg-[#2a0d10] text-[#8A9196] hover:text-[#c08080]"
                  title="Remove daily status"
                  data-testid="item-dialog-remove-daily"
                ><X size={12} /></button>
              </div>
            </Labeled>
          ) : (
            <Labeled label="Daily Use">
              <button
                onClick={() => apply({ ...draft, isDaily: true, isDailyUsed: false })}
                className="p-2 silver-border bg-[#0d0d0f] hover:bg-[#16161a] font-meta text-[10px] tracking-[0.2em] text-[#8A9196] flex items-center gap-1"
                data-testid="item-dialog-make-daily"
              ><Plus size={12} /> MAKE DAILY USE</button>
            </Labeled>
          )}
